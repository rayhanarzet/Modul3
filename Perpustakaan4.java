...
class BukuBaru {  // bagian ini berisi atribut Buku
    ...
    double hitungRoyaltiBaru(double hargaBuku) {  // ini merupakan method untuk menghitung royalti 10% dari harga asli
        return 0.10 * hargaBuku; // return 10% dari harga asli
    }
}

public class Perpustakaan4 {
    public static void main(String[] args) {
        ...
        BukuBaru buku1 = daftarBuku.get(0); // disini buat cek kesamaan ambil contoh perbandingan buku ke satu dan buku ke dua (index 0 dan index 1)
        BukuBaru buku2 = daftarBuku.get(1);

        System.out.println("Tingkat Kesamaan antara '" + buku1.judul + "' dan '" + buku2.judul + "': " + buku1.cekKesamaan(buku2) + "%");

        BukuBaru salinanBuku = buku1.salin(); // disini yang dijadikan contoh salinan adalah buku ke satu
        System.out.println("\nBuku yang telah disalin:");
        salinanBuku.tampilkanInfo();

        double hargaBuku = 230000;  // disini kita akan menghitung royalti
        double royalti = buku1.hitungRoyaltiBaru(hargaBuku);  // Panggil method yang sudah diubah namanya
        System.out.println("\nRoyalti untuk buku '" + buku1.judul + "' dengan harga Rp" + hargaBuku + " adalah: Rp" + royalti);
    }
}
